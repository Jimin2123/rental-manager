...
| 공통 이력 | `Attachment`, `AuditLog` | 첨부파일, 변경 이력 |
...
| 처리 | 이유 |
| --- | --- |
| 번호 채번 | `orderNo`, `quotationNo`, `contractNo`, `invoiceNo`, `paymentNo`, `refundNo`, `taxInvoiceNo`, `requestNo`, `managementNo` 생성 |
| 견적 전환 | `QuotationItem`을 `SaleOrderItem` 또는 `RentalOrderItem`으로 복사 |
| 판매 출고 | `Asset.status = SOLD`, `AssetEvent` 생성 |
| 렌탈 활성화 | `RentalContract.status`, `RentalContractItem.status`, `Asset.status = RENTED`, `AssetEvent` 생성 |
