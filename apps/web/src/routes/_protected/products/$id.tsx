...
import type { Product, AssetListItem, Supplier } from './-types';
  ...
  return (
    <div className="rounded-lg border bg-card p-4 space-y-3">
      <div className="flex justify-end gap-2">
        {!isEditing && (
          <Button size="sm" variant="outline" onClick={() => setIsEditing(true)}>수정</Button>
        )}
        <Dialog>
      ...
      {isEditing ? (
        <Form {...form}>
          <form onSubmit={form.handleSubmit((d) => void updateMutation.mutate(d))} className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" size="sm" onClick={() => setIsEditing(false)}>취소</Button>
        ...
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          <div><dt className="text-muted-foreground">제품명</dt><dd className="font-medium">{product.name}</dd></div>
          <div><dt className="text-muted-foreground">카테고리</dt><dd>{product.category ?? '-'}</dd></div>
          <div><dt className="text-muted-foreground">제조사</dt><dd>{product.manufacturer ?? '-'}</dd></div>
          <div><dt className="text-muted-foreground">모델명</dt><dd>{product.modelName ?? '-'}</dd></div>
          {product.memo && (
            <div className="col-span-2"><dt className="text-muted-foreground">메모</dt><dd>{product.memo}</dd></div>
          )}
        </dl>
      )}
    </div>
  ...
  return (
    <div className="rounded-lg border bg-card p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold">자산 목록</h2>
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          <DialogTrigger asChild>
            <Button size="sm" variant="outline">+ 자산 추가</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <Form {...addForm}>
              <form
                onSubmit={addForm.handleSubmit((d) => void addMutation.mutate(d))}
                className="space-y-3"
              >
                {/* 초기 상태 */}
                <FormField control={addForm.control} name="initialStatus" render={({ field }) => (
                  <FormItem>
                    <FormLabel>초기 상태 <span className="text-destructive">*</span></FormLabel>
                ...
                {/* 시리얼번호 */}
                <FormField control={addForm.control} name="serialNumber" render={({ field }) => (
                  <FormItem>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {/* 매입일 */}
                  <FormField control={addForm.control} name="purchaseDate" render={({ field }) => (
                    <FormItem>
                ...
                {/* 메모 */}
                <FormField control={addForm.control} name="memo" render={({ field }) => (
                  <FormItem>
                    <FormLabel>메모</FormLabel>
          ...
          {isLoading ? (
            ...
            assets.map((asset) => (
              <Fragment key={asset.id}>
                ...
                >
                  <TableCell>{asset.serialNumber ?? <span className="text-muted-foreground">S/N 미등록</span>}</TableCell>
                  <TableCell>
                ...
                {expandedId === asset.id && (
                  <TableRow>
                    <TableCell colSpan={4} className="p-0 bg-muted/30">
                      {/* Task 6에서 AssetPanel로 교체 */}
                      <div className="p-4 text-sm text-muted-foreground">자산 상세 정보 로딩 중...</div>
                    </TableCell>
                  </TableRow>
                )}
              </Fragment>
